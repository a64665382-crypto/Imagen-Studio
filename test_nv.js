...
async function testT2I_text_prompts() {
  const url = "https://ai.api.nvidia.com/v1/genai/stabilityai/stable-diffusion-3.5-large";
  const res = await fetch(url, {
    method: "POST",
    headers: { "Authorization": `Bearer ${apiKeyT2I}`, "Content-Type": "application/json", "Accept": "application/json" },
    body: JSON.stringify({ text_prompts: [{text: "A dog", weight: 1}] })
  });
  console.log("T2I text_prompts:", res.status, await res.text().catch(e=>e.message));
}

testT2I();
testT2I_text_prompts();
